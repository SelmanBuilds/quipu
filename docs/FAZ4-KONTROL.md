...
>
> **Tarih:** 2026-08-20 · **Durum:** İNCELENDİ (2026-08-21) — çıktı doğrulandı; ölçülen sayım düzeltmesi aşağıda işlendi.
>
...
- Test sayısı raporlanmış olmalı: FAZ 4'ün **7 yeni testi** (T-50…T-56) + baz 156 + main merge'iyle
  gelen T-45 → **final 164 iddia** (162 geçti + 2 skip; KONTROL'deki "157/164" sayım hatasıydı:
  ölçülen baz 154 geçti + 2 skip = 156, FAZ 3 raporuyla uyumlu). Mevcut testlerden silinmiş/

## 5. Regresyon kapısı (K-1)

- **Mevcut baz 156 iddia (154 geçti + 2 skip) aynen yeşil** — `capture`'ın Claude Code şeması değişmedi (K-1).
  Ölçülen final (main merge sonrası): 162 geçti + 2 skip = 164, 0 hata; diff'te silinen test yok (7 yeni `t` birebir).
...
8. **Regresyon sayısı:** ÇÖZÜLDÜ — gerçek sayı `sh tests/run.sh`'ten alındı: baz **156** (154+2), final **163**
   (161+2). KONTROL'deki "157" (155+2) ve "156 bayat" uyarısı birlikte hatalıymış: doğru baz 156 idi.
...
- [x] Dilim 0–3 eksiksiz; `FAZ4-BULGULAR.md` A-1…A-6'yı etiketli cevaplıyor
- [x] T-50…T-56 var ve güçlü; final 164 iddia (162 geçti + 2 skip); yerel yeşil, üç OS CI PR'da; shellcheck sessiz
- [x] K-1…K-25 birebir; `PreCompact`/`shell`/`env`/`conhost` adaptörde yok
- [x] Regresyon kapısı: 156 iddia aynen yeşil, silinen yok (spec'teki sayım 156'ya düzeltildi)
- [x] §9 yasak desenlerin hiçbiri yok
- [x] §7 kırmızı bayrakların 10'u temiz
- [x] K-23/K-24/K-25 belgeler tam; fixture'lar maskeli; her şey izleniyor
- [x] "Canlı Codex doğrulaması" `[doğrulanmadı]` olarak dürüstçe açık (K-24 risk satırı)
